...
However, further aspects that needs to be considered, in confirmatory hypothesis testing, e.g. correlation of subsequent bins, and that each day represents a repeated measurement of the same construct, sleep disturbance.
One approach to account for the repeated measurement nature is to model data in a hierarchical fashion, e.g. using linear mixed models. Yet, because of the scarcity of above zero values in predictor variables, there are to few observations per day, which precludes to model clustering in days. Accordingly, estimation procedures yield singularity warnings.
